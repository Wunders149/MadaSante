...
const endOfRoute = content.indexOf('})', end);
if (endOfRoute === -1) {
...
content = content.substring(0, start) + newRoute + content.substring(endOfRoute + '})'.length);
fs.writeFileSync('server/src/routes/admin.ts', content, 'utf8');
console.log('Restored route. New length:', content.length);
console.log('New route start:', content.substring(start, start + 90));
